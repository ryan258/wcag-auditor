...
- Auditor instance state: `base_url`, `max_depth`, `max_pages`, `timeout`, `user_agent`, `sample_strategy`, `base_domain`, `scheme`, `visited_urls`, `results`, `sampled_templates`, `wcag_rules`.
- User-pass runner state: resolved config, OpenRouter client, and per-run local findings only; no persistent cache or cross-run memory.
- Reporter instance state: `results` reference only; treat it as read-only input.
- Per-run mutable locals in `Auditor.audit()`: `urls_to_visit`, `queued_urls`, `pages_audited`, `all_violations`, `all_manual_reviews`, `all_warnings`, `all_passed`, `violation_types`, `manual_review_types`, `pages`, `page_insights`.
- Orthogonality rule: Playwright objects stay local to the audit layer and die within the call path; never store them in globals, reporter state, or returned payloads.
...
- Orthogonality rule: output formatting state stays in `Reporter`; `Auditor` and `UserPassRunner` must emit data, not markup.
...
- Mutation rule: treat the aggregate result dict as a snapshot after `audit()` returns; downstream code may create additive copies for optional layers like `user_pass`, but should not mutate the original in place.
...
- Aggregation rule: synthetic findings and rewrite suggestions live under `user_pass`; they must not backflow into `violations`, `manual_reviews`, or their summary counts.
